...
class CreationTableViewController: UITableViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UITextFieldDelegate {
    ...
    //private var stepImageArray = [UIImage?]()
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        if let image = info[UIImagePickerControllerEditedImage] as? UIImage {
            if picker == coverImagePicker {
                ...
                /*if setImageIndex >= stepImageArray.count {
                    while(stepImageArray.count <= setImageIndex) {
                        stepImageArray.append(nil)
                    }
                }*/
                //stepImageArray[setImageIndex] = image
  ...
  /* @IBAction func saveAsDraft(_ sender: UIButton) {
        if recipe == nil {
            container.performBackgroundTask { [weak self] context in
                let newRecipe = Recipe(context: context)
                if let image = self?.coverPhotoView.image {
                     newRecipe.coverPhoto = UIImagePNGRepresentation(image)! as NSData
                }
               
                //newRecipe.ingredients = (self?.ingredients)!
                //newRecipe.steps = (self?.step)!
                var newIngredientArray = [String?]()
                var newQuantityArray = [String?]()
                var newStepPhotoArray = [Data?]()
                var newStepDetailArray = [String?]()
                var i = 0
                if let count = self?.ingredients.count {
                    while( i < count) {
                        newIngredientArray.append(self?.ingredients[i].0)
                        newQuantityArray.append(self?.ingredients[i].1)
                        i += 1
                    }
                }
                i = 0
                if let count = self?.step.count {
                    while( i < count) {
                        newStepPhotoArray.append(self?.step[i].0)
                        newStepDetailArray.append(self?.step[i].1)
                        i += 1
                    }
                }
    ...
    @IBAction func saveRecipe(_ sender: UIButton) {
        if recipe == nil {
            container.performBackgroundTask { [weak self] context in
                ...
                newRecipe.name = self?.recipeNameTextField.text
                //newRecipe.ingredients = (self?.ingredients)!
                //newRecipe.steps = (self?.step)!
                var newIngredientArray = [String?]()
                var newQuantityArray = [String?]()
